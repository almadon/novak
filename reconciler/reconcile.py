...
# Values that mean "nobody has filled this in yet". Kept in step with
# PLACEHOLDERS in scripts/lib/vars.sh.
PLACEHOLDERS = {"EDIT-ME", "set-in-keychain", "changeme", ""}


def configured_vars() -> set[str]:
    """
    Names of variables that hold a real value, from the host environment and
    .env. Only presence is determined here: values are tested against the
    placeholder list and then discarded, never stored, rendered or logged. The
    invariant that this script does not handle secret values still holds.
    """
    found = {k for k, v in os.environ.items() if v.strip() not in PLACEHOLDERS}
    ...
    if env_file.exists():
        for line in env_file.read_text().splitlines():
            ...
            key = key.strip()
            if ENV_RE.match(key) and value.strip().strip("\"'") not in PLACEHOLDERS:
                found.add(key)
    return found
...
def missing_vars(server: dict, configured: set[str]) -> list[str]:
    """Declared variables for this server that nobody has filled in."""
...
def render(servers: list[dict], configured: set[str]) -> dict:
    ...
    for s in servers:
        ...
        if s["kind"] == "external" or not s["enabled"]:
            continue
        # An unconfigured integration is skipped, not fatal. Rendering it anyway
        # would start a container with empty variables that fails in a way that
        # is much harder to read than a line saying which variable is missing.
        # One unconfigured MCP server must never stop the rest of the stack.
        if missing_vars(s, configured):
            continue
        svc = {
            "image": s["image"],
...
def main() -> int:
    ...
    rendered = yaml.safe_dump(render(servers, configured), sort_keys=False)
    header = (
